...
export default async function StorePage({
  ...
  function renderSection(sec: SectionRow) {
    ...
    switch (sec.type) {
      ...
      case "products":
        return (
          <div key={sec.id} id="shop" className={`${containerMax} mx-auto px-4 ${sectionPadding} scroll-mt-20`}>
  ...
  const storeNav = (
    <nav
      className={`sticky top-0 z-40 border-b backdrop-blur-xl ${s ? "" : "bg-white/90 dark:bg-[#0f0f0f]/90 border-gray-100 dark:border-white/10"}`}
      style={s ? { background: `${bgColor}e6`, borderColor: `${textColor}10` } : undefined}
    >
      <div className={`${containerMax} mx-auto px-4 py-3 flex items-center justify-between gap-3`}>
        ...
        <div className="flex items-center gap-1 text-sm shrink-0">
          <a
            href="#shop"
            className="px-3 py-1.5 rounded-lg font-medium text-gray-600 hover:opacity-70 transition-opacity"
            style={s ? { color: textColor } : undefined}
          >
            Shop
          </a>
  ...
  return (
    ...
    >
      ...
      {/* Products */}
      <div id="shop" className={`${containerMax} mx-auto px-4 ${sectionPadding} scroll-mt-20`}>
        {(!products || products.length === 0) ? (
          <div className="text-center py-12">
            <EmptyIllustration variant="store" className="opacity-90 mb-2 max-w-[360px] mx-auto" />
